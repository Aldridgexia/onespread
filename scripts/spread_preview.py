...
def preview():
    ...
    for long in contracts:
        ...
        if not short or any(not c["tradable"] or str(c["size"]) != "100"
                            or c.get("root_symbol") != "SPY" or c.get("underlying_symbol") != "SPY"
                            for c in (long, short)):
            continue
        ...
        if any(not number(s.get("impliedVolatility")) or s["impliedVolatility"] <= 0 for s in legs):
            continue
        if any(not all(number((s.get("greeks") or {}).get(k)) for k in REQUIRED_GREEKS) for s in legs):
            continue
        if not .40 <= abs(legs[0]["greeks"]["delta"]) <= .65:
            continue
        quotes = [s.get("latestQuote") or {} for s in legs]
        if any(not (number(q.get("bp")) and number(q.get("ap")) and q["ap"] >= q["bp"] > 0 and q.get("t")) for q in quotes):
            continue
        ...
        debit = (Decimal(str(quotes[0]["ap"])) - Decimal(str(quotes[1]["bp"]))).quantize(Decimal(".01"), rounding=ROUND_CEILING)
        if not Decimal(0) < debit < Decimal(5) or debit * 100 > 300:
            continue
        kind = "bull_call" if long["type"] == "call" else "bear_put"
        digest = hashlib.sha256((long["symbol"] + short["symbol"] + quotes[0]["t"]).encode()).hexdigest()[:20]
        ...
        blockers += ["saved_account_state_requires_refresh", "no_current_regime_or_event_assessment"]
        payload = {"order_class": "mleg", "qty": "1", "type": "limit", "time_in_force": "day",
                   "limit_price": str(debit), "client_order_id": "preview-" + digest,
                   "legs": [{"symbol": long["symbol"], "ratio_qty": "1", "side": "buy", "position_intent": "buy_to_open"},
                            {"symbol": short["symbol"], "ratio_qty": "1", "side": "sell", "position_intent": "sell_to_open"}]}
        candidates.append({"id": digest, "kind": kind, "expiration": long["expiration_date"], "dte_at_snapshot": dte,
                           "long_strike": str(strike), "short_strike": str(short_strike), "net_debit": str(debit),
                           "premium_usd": str(debit * 100), "expiration_max_profit_before_costs_usd": str((5-debit)*100),
                           "greeks_per_share": {k: round(legs[0]["greeks"][k]-legs[1]["greeks"][k], 6) for k in REQUIRED_GREEKS},
                           "quote_timestamps": [q["t"] for q in quotes], "blockers": blockers,
                           "selection_distance": abs(abs(legs[0]["greeks"]["delta"])-.5) + abs(dte-9)*.02,
                           "order_draft": payload})
    candidates.sort(key=lambda c: c["selection_distance"])
    chosen = [next(c for c in candidates if c["kind"] == kind) for kind in ("bull_call", "bear_put")
              if any(c["kind"] == kind for c in candidates)]
    result = {"generated_at": now.isoformat(), "mode": "offline_preview", "decision": "WAIT",
              "reason": "Saved after-hours data cannot authorize an order; refresh market/account state and assess regime first.",
              "feed": "indicative", "mathematically_eligible_drafts": len(candidates), "examples": chosen,
    ...
    print(json.dumps({"decision": result["decision"], "draft_count": len(candidates),
                      "examples": [{k: c[k] for k in ("kind", "expiration", "long_strike", "short_strike", "premium_usd", "blockers")} for c in chosen]}, indent=2))
