...
def config():
    ...
    return {k: values[k] for k in ("FEATHERLESS_API_KEY", "LLM_BASE_URL", "LLM_MODEL", "LLM_COMPARISON_MODEL")}
...
def request(path, payload=None):
    ...
    req = Request("https://api.featherless.ai/v1" + path,
                  data=json.dumps(payload).encode() if payload is not None else None,
                  headers={"Authorization": "Bearer " + settings["FEATHERLESS_API_KEY"],
                           "Content-Type": "application/json", "User-Agent": "OneSpread-eval/0.1"})
    started = time.monotonic()
    try:
        with build_opener(NoRedirect).open(req, timeout=60) as response:
            value = json.load(response)
    except HTTPError as error:
        message = error.read().decode(errors="replace").replace(settings["FEATHERLESS_API_KEY"], "[REDACTED]")
...
def inspect_access():
    ...
    for model in (settings["LLM_MODEL"], settings["LLM_COMPARISON_MODEL"]):
        ...
        fields = ("id", "available_on_current_plan", "is_gated", "context_length", "max_completion_tokens",
                  "pricing", "features", "status", "availability", "concurrency_cost")
