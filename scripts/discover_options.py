...
from alpaca_access import PRIVATE, ROOT, get, save
...
def inspect_symbol(symbol, start, end, stock):
    ...
    contracts = pages(
        ...
        dict(params, underlying_symbols=symbol, status="active", limit=10000),
        "option_contracts",
    )
    ...
    snapshots = pages(
        ...
        dict(params, feed="indicative", strike_price_gte=low, strike_price_lte=high, limit=1000),
        "snapshots",
        data=True,
    )
    ...
    for snap in snapshots.values():
        quote, trade, greeks = (
            snap.get("latestQuote") or {},
            snap.get("latestTrade") or {},
            snap.get("greeks") or {},
        )
    ...
    summary = {
        ...
        "reference_bar": stock.get("dailyBar"),
        "strike_band": [low, high],
        ...
        "near_money_contracts": len(expected),
        "snapshots": len(snapshots),
        "near_money_contracts_with_snapshot": sum(c["symbol"] in snapshots for c in expected),
        "quotes": quote_count,
        "positive_two_sided_quotes": two_sided,
        "positive_iv": iv_count,
        "all_five_greeks": greek_count,
        "greek_fields": dict(greek_fields),
...
def scan():
    ...
    start = date.fromisoformat(
        (clock["timestamp"] if clock["is_open"] else clock["next_open"])[:10]
    )
    end = start + timedelta(days=45)
    stock_snapshots = get(
        "/v2/stocks/snapshots", {"symbols": ",".join(SYMBOLS), "feed": "iex"}, data=True
    )
    save("stock_snapshots_iex.json", stock_snapshots)
    summary = {
        "captured_at": datetime.now(timezone.utc).isoformat(),
        "clock": clock,
        "expiration_gte": str(start),
        "expiration_lte": str(end),
        "days": 45,
        "stock_feed": "iex",
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        tasks = {
            pool.submit(inspect_symbol, s, str(start), str(end), stock_snapshots.get(s, {})): s
            for s in SYMBOLS
        }
    ...
    try:
        get(
            ...
            {"feed": "opra", "limit": 1, "expiration_date_gte": str(start)},
            data=True,
        )
    ...
    write_csv(
        output / "option_underlyings.csv",
        sorted(assets, key=lambda a: a["symbol"]),
    ...
    write_csv(
        output / "option_contracts_45d.csv",
        sorted(contracts, key=lambda c: c["symbol"]),
        [
            "symbol",
            "underlying_symbol",
            "root_symbol",
        ...
        json.dumps(
            {
                ...
                "open_orders": summary["open_orders_count"],
                "opra_latest_access": summary["opra_latest_access"],
            },
            indent=2,
        ),
        flush=True,
...
def probe_history():
    ...
    for name, path, extra in [
        ...
        if (
            name == "bars"
            and all(prior.get(k) == result[k] for k in ("symbol", "start", "end"))
        ...
        try:
            response = get(
                path,
                dict(symbols=sample["symbol"], start=start, end=end, limit=5, sort="desc", **extra),
                data=True,
            )
